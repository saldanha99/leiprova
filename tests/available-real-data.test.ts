...
describe("carga de dados reais disponíveis", () => {
  it("mantém duas edições atuais vinculáveis e duas históricas oficiais", () => {
    expect(validateAvailableRealExamEditions()).toHaveLength(4);
    ...
    expect(scheduled.every((edition) => edition.opportunitySlug && edition.productSlug)).toBe(true);
  ...
  it("aceita somente fontes oficiais HTTPS da FGV e não incorpora conteúdo", () => {
    for (const edition of AVAILABLE_REAL_EXAM_EDITIONS) {
      expect(new URL(edition.officialUrl).hostname).toBe("conhecimento.fgv.br");
