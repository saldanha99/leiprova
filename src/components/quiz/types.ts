...
export function isQuizConfigReady(config: QuizConfig) {
  ...
  if (config.path === "bank") {
    return Boolean(config.bankSlug && config.subjectSlug);
  }
  ...
  if (config.mode === "previous_exam" && config.examScope === "latest") return true;
  if (config.mode === "original_style") return Boolean(config.bankSlug && config.subjectSlug);
